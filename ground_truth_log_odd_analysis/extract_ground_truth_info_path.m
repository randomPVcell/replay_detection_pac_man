function [log_odd] = extract_ground_truth_info_path(folders,option,method,shuffles,ripple_zscore_threshold,p_val_threshold,suffix)
% Input:
...
%     load('extracted_place_fields_BAYESIAN.mat');
    ...
    if strcmp(option,'original') || strcmp(option,'common')
        ...
        [p_values, replay_scores] = extract_score_and_pvalue_ground_truth(scored_replay, scored_replay1_path, scored_replay2_path, method, [],suffix);
        destination = pwd; 
        [significant_replay_events sig_event_info] = number_of_significant_replays_ground_truth_path(p_val_threshold,ripple_zscore_threshold,method,shuffles,[],destination,suffix);

        % gather time information for sorting the event as PRE, POST or RUN
        [~,time_range]=sort_replay_events([],'path',suffix);
        ...
        % gather time information for sorting the event as PRE, POST or RUN
        [~,time_range]=sort_replay_events([],suffix);
