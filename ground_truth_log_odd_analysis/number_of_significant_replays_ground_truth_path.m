function [significant_replay_events sig_event_info] = number_of_significant_replays_ground_truth_path(p_value_threshold,ripple_zscore_threshold, method, shuffles,rexposure_option,destination,suffix)
...
cd(destination)
load(fullfile(pwd,'extracted_replay_events'));
load(fullfile(pwd,'extracted_position'));
load(fullfile(pwd,'extracted_sleep_state'));
load(fullfile(pwd,'decoded_replay_events'));
load(fullfile(pwd,'decoded_replay_events_segments'));
...
[p_values, replay_scores] = extract_score_and_pvalue(scored_replay, scored_replay1_path, scored_replay2_path, method, replay_above_rippleThresh_index,suffix);
...
for track = 1 :  number_of_tracks % for each track
    ...
    for event = 1 : number_of_events %for each event
        ...
        elseif strcmp(shuffles,'PRE place and POST time')
            % if Only PRE place field circular shift and POST place bin
            % shift
            sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,[2 4]))<p_value_threshold | max(p_values.FIRST_HALF(track,event,[1 2]))<p_value_threshold/2 | max(p_values.SECOND_HALF(track,event,[1 2]))<p_value_threshold/2; % indices - 0 or 1 for significant event
...
% Save
    if ~isempty(rexposure_option) && rexposure_option == 2
        save(sprintf('significant_replay_events_%s_individual_exposures.mat', suffix), 'significant_replay_events')

    else
        save(sprintf('significant_replay_events_%s.mat', suffix), 'significant_replay_events')
    end        
...
function [p_values, replay_scores] = extract_score_and_pvalue(scored_replay_path, scored_replay1_path, scored_replay2_path, method, replay_above_rippleThresh_index,suffix)
% INPUTS:
...
number_of_events = length(scored_replay_path(1).replay_events);

for track = 1 :  number_of_tracks % for each track
    for event = 1 : number_of_events %for each event
        p_values.WHOLE(track,event,:) = scored_replay_path(track).replay_events(event).p_value_path_new;
        replay_scores.WHOLE(track,event) = scored_replay_path(track).replay_events(event).(suffix);
        p_values.FIRST_HALF(track,event,:) = scored_replay1_path(track).replay_events(event).p_value_path_new;
        replay_scores.FIRST_HALF(track,event) = scored_replay1_path(track).replay_events(event).(suffix);
        p_values.SECOND_HALF(track,event,:) = scored_replay2_path(track).replay_events(event).p_value_path_new;
        replay_scores.SECOND_HALF(track,event) = scored_replay2_path(track).replay_events(event).(suffix);
    end
end
