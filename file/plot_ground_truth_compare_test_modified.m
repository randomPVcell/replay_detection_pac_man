...
for nmethod = 1:length(method)
    ...
    elseif strcmp(method{nmethod},'linear_within')
        load log_odd_linear_PRE_place_POST_time_within.mat
        %         load log_odd_linear_place_bin_circular_shift
        log_odd_compare{nmethod} = log_odd;

    elseif strcmp(method{nmethod},'path_updated')
        load log_odd_path_updated_PRE_place_POST_time.mat
        log_odd_compare{nmethod} = log_odd;
    

    elseif strcmp(method{nmethod},'path_within')
        load log_odd_path_within_PRE_place_POST_time.mat
        log_odd_compare{nmethod} = log_odd;
...
if exist(sprintf('log_odd_difference_compare_test_%s.mat',suffix), 'file') ~= 2
    for epoch = 1:3
        for nmethod = 1:4
            tic
    ...
    save(sprintf('log_odd_difference_compare_test_%s.mat',suffix), 'log_odd_difference', 'log_odd_difference_CI', 'percent_sig_events', 'percent_multi_events', 'percent_sig_events_CI', 'percent_multi_events_CI')
    ...
    load(fullfile(pwd, sprintf('log_odd_difference_compare_test_%s.mat', suffix)));
end
 

if exist('log_odd_difference_compare_test__%s_original.mat', 'file') ~= 2
    for epoch = 1:3
        for nmethod = 1:length(method)
            tic
            total_number(epoch) = length(epoch_index{nmethod}{epoch});
    ...
    save(sprintf('log_odd_difference_compare_test_original_%s.mat',suffix), 'log_odd_difference_original', 'percent_sig_events_original', 'percent_multi_events_original')
    % clear boot_log_odd_difference boot_percent_sig_events boot_percent_multi_events
    
else
    load(fullfile(pwd, sprintf('log_odd_difference_compare_test_original_%s.mat',suffix)));
...
% colour_line = {[69,117,180]/255,[244,165,130]/255,[215,48,39]/255};
 colour_line= {[8,81,156]/255,[103,174,110]/255,[244,109,67]/255,[215,48,39]/255};
...
filename = sprintf('%s test comparision original_%s.pdf',method{nmethod},suffix)
cd Figure_new
saveas(gcf,filename)
cd ..
...
filename = sprintf('%s test comparisions CI_%s.pdf',method{nmethod},suffix)
cd Figure_new
saveas(gcf,filename)
cd ..
